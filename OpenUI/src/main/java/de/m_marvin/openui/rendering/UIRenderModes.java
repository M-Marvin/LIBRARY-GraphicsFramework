...
public class UIRenderModes<R extends IResourceProvider<R>> {
	...
	public static <R extends IResourceProvider<R>> RenderMode<R> solidPlane(R shaderLocation, R shaderLibLocation) {
		return new RenderMode<R>(RenderPrimitive.TRIANGLES, new VertexFormat().appand("position", NumberFormat.FLOAT, 3, false).appand("color", NumberFormat.FLOAT, 4, false), shaderLocation, shaderLibLocation, (shader, tl) -> {
